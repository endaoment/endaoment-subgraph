...
describe('Migration Detection Tests', () => {
  ...
  describe('Transfer events', () => {
    ...
    test('it should finalize migration on transfer destination (has v1 funds)', () => {
      ...
      // Block 1 (ORG2):
      // - V1 Assets: 300 USD
      // - 1 Transfer in of 100 USD | 0.5 USD fee | Balance = 299.50 USD
    ...
    test('it should finalize migration on transfer destination (no v1 funds)', () => {
      ...
      // Block 1 (ORG2):
      // - V1 Assets: 0 USD
      // - 1 Transfer in of 100 USD | 0.5 USD fee | Balance = 99.50 USD
      // - Balance at the end of block: 99.50 USD
      mockBalance(DEFAULT_ORG_ADDRESS, 0)
      mockBalance(DEFAULT_ORG2_ADDRESS, 99_500_000)
      handleEntityValueTransferred(
        createDefaultValueTransferredEvent(DEFAULT_ORG_ADDRESS, DEFAULT_ORG2_ADDRESS, 100_000_000),
      )
      ...
      // Block 2:
      // - 1 donation of 150 USD | 0.75 USD fee | Balance = 248.75 USD
      mockBalance(DEFAULT_ORG2_ADDRESS, 248_750_000)
      handleEntityDonationReceived(createDefaultDonationEvent(DEFAULT_ORG2_ADDRESS, 150_000_000, 2))
      const entity = NdaoEntity.load(DEFAULT_ORG2_ADDRESS)
  ...
  test('it should correctly index balance reconciled with V1 Assets', () => {
    // ------ Act -------
    // Block 1:
    // - 1 donations of 200 USD
    // - Fees = 1.0 USD (0.5%)
    ...
    // - Balance at the end of block: 1399,00 USD
    mockBalance(DEFAULT_ORG_ADDRESS, 1_399_000_000)
    handleEntityBalanceReconciled(createDefaultBalanceReconciledEvent(DEFAULT_ORG_ADDRESS, 200_000_000))
  ...
  test('it should correctly index payout event from an entity with V1 Assets', () => {
    // ------ Act -------
    // Block 1:
    // - 1 payout - 1 for 400 USD
    // - Fees = 2 USD (0.5%)
    // - V1 Migrated Assets = 400 USD
    // - Balance at the end of block: 0 USD
    mockBalance(DEFAULT_ORG_ADDRESS, 0)
    handleEntityValuePaidOut(createDefaultValuePaidOutEvent(DEFAULT_ORG_ADDRESS, EXTERNAL_ADDRESS, 400_000_000))
    ...
    assert.bigIntEquals(BigInt.fromI32(0), entity.totalUsdcTransferredOut)
    assert.bigIntEquals(BigInt.fromI32(0), entity.totalUsdcTransferredOutFees)
    assert.bigIntEquals(BigInt.fromI32(398_000_000), entity.totalUsdcPaidOut)
    assert.bigIntEquals(BigInt.fromI32(2_000_000), entity.totalUsdcPaidOutFees)
    assert.booleanEquals(true, entity.initialized)
  })

  test('it should correctly index deposits from an entity with V1 Assets', () => {
    // ------ Arrange ------
