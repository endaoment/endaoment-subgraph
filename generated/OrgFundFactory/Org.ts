...
export class Org extends ethereum.SmartContract {
  static bind(address: Address): Org {
    return new Org("Org", address);
  }
  ...
  orgId(): Bytes {
    let result = super.call("orgId", "orgId():(bytes32)", []);

    return result[0].toBytes();
  }

  try_orgId(): ethereum.CallResult<Bytes> {
    let result = super.tryCall("orgId", "orgId():(bytes32)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
...
export class SetOrgIdCall extends ethereum.Call {
  get inputs(): SetOrgIdCall__Inputs {
    return new SetOrgIdCall__Inputs(this);
  }

  get outputs(): SetOrgIdCall__Outputs {
    return new SetOrgIdCall__Outputs(this);
  }
}

export class SetOrgIdCall__Inputs {
  _call: SetOrgIdCall;

  constructor(call: SetOrgIdCall) {
    this._call = call;
  }
...
export class SetOrgIdCall__Outputs {
  _call: SetOrgIdCall;
